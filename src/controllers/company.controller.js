...
export const create = asyncHandler(async function (req, res, _next) {
  const createdDoc = await DepartmentService.create(req.body);
  sendResponse(
    httpStatus.CREATED,
    res,
    createdDoc,
...
export const update = asyncHandler(async function (req, res, _next) {
  const { id } = req.params;
  const updatedDoc = await DepartmentService.update(id, req.body);
...
export const deleteData = asyncHandler(async function (req, res, _next) {
  const { id } = req.params;
  const deletedDoc = await DepartmentService.deleteDoc(id);
